    ...
    if not valid_dishes:
        ...
        if total_ingredients_extracted:
            ...
            if categorized_and_normalized:
                ...
                st.markdown("---")
                st.subheader("Send to Google Keep (via Email)")
                send_to_keep = st.checkbox("Do you want to send this shopping list to Google Keep?")

                if send_to_keep:
                    st.info(
                        "Google Keep allows you to email notes directly to your Keep account! "
                        "Simply send an email to `your-google-account-username@keep.google.com` "
                        "(e.g., `janedoe@keep.google.com`). "
                        "The subject will be the note title, and the email body will be the content."
                    )
                    google_account_email = st.text_input("Please enter your Google account email address (e.g., `janedoe@gmail.com`) for the `keep.google.com` address:", key="google_email_input")

                    if google_account_email:
                        # Construct the mailto link
                        keep_email_address = f"{google_account_email.split('@')[0]}@keep.google.com" if '@' in google_account_email else f"{google_account_email}@keep.google.com"
                        email_subject = urllib.parse.quote_plus("Grocery List from AI Agent")
                        email_body = urllib.parse.quote_plus(f"Here is your generated grocery list:\n\n{grocery_list_text}\n\nGenerated by the AI Grocery List Generator.")

                        mailto_link = f"mailto:{keep_email_address}?subject={email_subject}&body={email_body}"

                        st.markdown(
                            f"Click the button below to open your email client with the list pre-filled. "
                            f"Make sure the 'To' address is set to your Google Keep email: `{keep_email_address}`."
                        )
                        st.markdown(f'<a href="{mailto_link}" target="_blank" class="button-link"><button style="background-color:#4CAF50;color:white;padding:10px 20px;border:none;border-radius:5px;cursor:pointer;font-size:16px;">Open Email Client with List</button></a>', unsafe_allow_html=True)
                        st.markdown(
                            "**Note:** Your browser might ask for permission to open your default email application. "
                            "You can then send the email, and it should appear as a new note in your Google Keep!"
                        )
                    else:
                        st.warning("Please enter your Google account email address to generate the email link.")
...
st.sidebar.markdown(
    ...
    "Direct API integration with Google Keep is complex for a purely client-side Streamlit app, "
    "so this agent guides you to use Google Keep's email-to-note feature."
)
